...
class BlogSpider(scrapy.Spider):
    ...
    def parse(self, response):          # response = webpage
        #loop through current page
        for href in response.css('.smw-columnlist-container')[0].css("a::attr(href)").extract():             # filter evrything that have class="smw-columnlist-container"
            extracted_data = scrapy.Request("https://naruto.fandom.com"+href, callback = self.parse_jutsu)
            yield extracted_data

        # loop through next page
